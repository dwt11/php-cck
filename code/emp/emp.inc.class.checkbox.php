 ...
 * 员工选择CHECKBOX
 *
 * @version        $Id: empMapCheckbox.class.admin.php 1 15:21 5日
 * @package
 * @copyright
 * @license
 * @link
...
class empMapCheckbox
{
    ...
    function GetTotalEmp($depId)
    {
      ...
      $total="";//150130原来注销
        ...
        {$id=$row['emp_id'];
		   ...
		   <input type='checkbox' name='selempid' id='selempid{$id}' value='{$id}'  />".GetIntAddZero($row['emp_code'])."-".$row['emp_realname']."</label>
		   </div>\r\n";
           // $total = $row['dd'];
        }
	        //($total == "")$total = "0";
    ...
    function empAllCheckbox()
    {
	   ...
	   //返回的权限查询语句,里面包含所有的可以查询的ID,所以在获取子分类时 要检测是否已经查询过此ID
		...
		if($wheresql==""){$wheresql=" and dep_reid=0";}
        ...
        while($row = $this->dsql->GetObject(0))
        {
			...
			echo"<br>".$this->GetTotalEmp($dep_id)."  ";
    ...
    function LogicListAllSunDep($id, $step)
    {
        ...
        $fid = $id;
        $this->dsql->SetQuery("SELECT * FROM `#@__emp_dep` WHERE dep_reid='".$id."' ORDER BY dep_id");
        ...
        if($this->dsql->GetTotalRow($fid)>0)
        {
            while($row = $this->dsql->GetObject($fid))
            {
				...
				echo"<br>$step".$this->GetTotalEmp($id)."  ";
