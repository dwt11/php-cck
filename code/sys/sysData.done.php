 ...
 * 数据库操作
 *
 * @version        $Id: sysData.done.php 1 17:19 20日
 * @package
 * @copyright
 * @license
 * @link
...
if($dopost=='bak')
{
    ...
    if(!is_dir($bkdir))
    {
        MkdirAll($bkdir, $cfg_dir_web_role);
        CloseFtp();
    }
    ...
    if($nowtable=='')
    {
        ...
        $doneForm = "<form name='gonext' method='post' action='sysData.done.php'>
    ...
    else
    {
        ...
        $tmsg = "<font color='red'>完成到{$m}条记录的备份，继续备份{$nowtable}...</font>";
        $doneForm = "<form name='gonext' method='post' action='sysData.done.php?dopost=bak'>
          <input type='hidden' name='isstruct' value='$isstruct' />
          <input type='hidden' name='fsize' value='$fsize' />
          <input type='hidden' name='tablearr' value='$tablearr' />
          <input type='hidden' name='nowtable' value='$nowtable' />
...
else if($dopost=='redat')
{
    ...
    if($startgo==0 && $structfile!='')
    {
        ...
        $doneForm = "<form name='gonext' method='post' action='sysData.done.php?dopost=redat'>
...
{
    global $cfg_dir_web_role,$cfg_soft_lang;
    /*$msginfo = "<html>\n<head>
    ...
    $msginfo="<!DOCTYPE html>
                ...
                <div class='wrapper wrapper-content animated fadeInRight'>
                    <div style='width: 33%; min-width: 200px;text-align:center; margin:0 auto;text-align: left'>
                        <div class='panel panel-info'>
                            <div class='panel-heading'>
                                <i class='fa fa-info-circle'></i> <b>提示信息</b>
                            </div>
