...
class DeviceTypeUnit
{
    ...
    function __construct($typeid = 0)
    {
        ...
        //载入单条类目信息
        $query = "SELECT tp.* FROM `#@__device_type` tp WHERE tp.id='$typeid'";
        //dump($query);
        if ($typeid > 0) {
            $row = $this->dsql->GetOne($query);
            $this->ispart = $row['ispart'];//当前浏览的分类页面  是否频道封面，在device.PHP页面中使用(频道页面不可以添加)
        }
        $this->getAllTypeInfoToArray();//获取所有分类信息多维数组
    }

    function DeviceTypeUnit($typeid)
    {
    ...
    {
        $query = "SELECT id,reid,typename,ispart FROM `#@__device_type`  ORDER BY id,sortrank asc ";
     ...
     * @return 多个option字符串
     */
    function GetDeviceTypeOptionS($GetSonTypeId=0)
    {
        //$returnStr = "";
    ...
    {
        $this->dsql = $GLOBALS['dsql'];
        $this->dsql->SetQuery("SELECT typeid,count(typeid) as dd FROM `#@__device`  group by typeid");
        $this->dsql->Execute();
        while ($row = $this->dsql->GetArray()) {
            $this->catalogDeviceNumsArray[$row['typeid']] = $row['dd'];
        }
    }

    /**
     * 170106
     * 商品列表页 device_del.php使用,判断删除的分类是否包含商品
     * 获取指定分类下的商品数量
     *
     * @param $tid
     *
     * @return int|mixed
     */
    function GetTotalDevice($tid)
    {
        // dump($tid);
        if (!is_array($this->catalogDeviceNumsArray)) {
            $this->UpdateCatalogNum();
        }
        if (!isset($this->catalogDeviceNumsArray[$tid])) {
            return 0;
        } else {
            $totalnum = 0;
            ...
            foreach ($ids as $tid) {
                if (isset($this->catalogDeviceNumsArray[$tid])) {
                    $totalnum += $this->catalogDeviceNumsArray[$tid];
                }
            }
            return $totalnum;
        }
