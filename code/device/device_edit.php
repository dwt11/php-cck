 ...
 * 商品编辑
 *
 * @version        $Id: device_edit.php 1 8:26 2010年7月12日
 * @package
 * @license
 * @link
 */
...
$query = "SELECT device.*,gt.channeltype as channelid FROM `#@__device` device
    LEFT JOIN `#@__device_type` gt ON gt.id=device.typeid
 WHERE device.id='$id' ";
...
$deviceoldcode = $goodRow['devicecode'];//用于保存的时候判断用户是否修改了编号,如果修改了编号要判断新的编号是否与数据库冲突
...
if ($dopost == 'save') {
    ...
    if ($addtable != '' && $inadd_f != '') {
        $iquery = "UPDATE `$addtable` SET {$inadd_f} WHERE deviceid='$id'";
        //dump($iquery);
...
if ($dopost != 'save') {
    ...
    $addRowAddtable = $dsql->GetOne("SELECT * FROM `$addtable` WHERE deviceid='$id'");
    //dump($addRowAddtable);
   ...
   /* $lyht_option="";
    ...
    $dpl = new DWTTemplate();
    //模板
    if (empty($edittemp)) $edittemp = 'device_edit.htm';
     //$edittemp = 'device_edit.htm';
    $tpl = $edittemp;
    //dump($tpl);
