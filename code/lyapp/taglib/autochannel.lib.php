 ...
 * 按排序位置的获取单个栏目的链接信息
 *
 * @version        $Id: autochannel.lib.php 1 9:29 6日
...
<demo>
{dwt:autochannel partsort='' typeid=’’}{/dwt:autochannel}
</demo>
<attributes>
...
function lib_autochannel(&$ctag,&$refObj)
{
    ...
    $row = $dsql->GetOne("SELECT id,typename FROM `#@__archives_type` WHERE reid='{$topid}' AND 
    ...
    $row = $dsql->GetOne("SELECT id,ispart  
                          FROM `#@__archives_type` WHERE id='$typeid' ");
    if(!is_array($row)) return '';
    ...
    {
        $row['typelink'] = GetArchiveTypeUrl($row['id']);
