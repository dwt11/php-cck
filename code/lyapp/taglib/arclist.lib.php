 ...
 * 文章列表调用标记
 *
 * 9:19 13日:修正对isweight属性的支持
 *
 * @version        $Id: arclist.lib.php 3 9:19 13日
...
<demo>
{dwt:arclist  flag='h' typeid='' row='' col='' titlelen='' infolen='' imgwidth='' imgheight='' listtype='' orderby='' keyword='' limit='0,1'}
        ...
        $innertext='', $arcid=0, $idlist='', $channelid=0, $limit='', $att='', $order='desc', $subday=0, $noflag='',$tagid='', $pagesize=0)
{
    ...
    if($idlist=='')
    {
        ...
        if($writer=='this') {
            $wuserid =  isset($refObj->Fields['userid']) ? $refObj->Fields['userid'] : 0;
            $orwheres[] = " arc.userid = '$wuserid' ";
        }
        ...
        if($att != '') {
            $flags = explode(',', $att);
            for($i=0; isset($flags[$i]); $i++) $orwheres[] = " (FIND_IN_SET('{$flags[$i]}', arc.flag)>0  and arc.litpic!='' and arc.litpic not like '%http%')";//141217修改 查询图片不为空的&& 不包含HTTP的(只站内图片)
        }


	//dump($typeid);
        if(!empty($typeid) && $typeid != 'top')    //141104注销 原为首页如果填写上typeid得不到当前分类的数据
        {
            //指定了多个栏目时，不再获取子类的id
            if( preg_match('#,#', $typeid) )
            {
                ...
                if($getall==1 || empty($refObj->Fields['typeid']))
                {
                    ...
                    foreach($typeids as $ttid) {
                        $typeidss[] = GetArchiveSonIds($ttid);
                    }
			...
			{
				$orwheres[] = ' arc.typeid IN ('.GetArchiveSonIds($typeid).')';
				}
        }
        ...
        if(!empty($channelid)) $orwheres[] = " And arc.channelid = '$channelid' ";
    ...
    else if($orderby == 'sortrank' || $orderby=='senddate') $ordersql = "   ORDER BY   arc.sortrank $orderWay";
    ...
    $query = "SELECT arc.*,tp.typename,tp.ispart 
        $addfieldsSql
        FROM `$maintable` arc LEFT JOIN `#@__archives_type` tp on arc.typeid=tp.id
        $addfieldsSqlJoin
    ...
    for($i=0; $i<$line; $i++)
    {
        ...
        for($j=0; $j<$col; $j++)
        {
            ...
            if($row = $dsql->GetArray("al"))
            {
                ...
                $row['arcurl'] =  $GLOBALS['cfg_install_path']."/web/archives_view.php?aid=".$row['id']; //列表中文章的地址
                $row['typeurl'] = $GLOBALS['cfg_install_path']."/web/archives_list.php?tid=".$row['typeid'];

                if($row['litpic'] == '-' || $row['litpic'] == '')
                {
                    $row['litpic'] =$GLOBALS['cfg_install_path'].'/images/defaultpic.gif';
                }
...
function lib_GetAutoChannelID($sortid, $topid)
{
    ...
    $getstart = $sortid - 1;
    $row = $dsql->GetOne("SELECT id,typename FROM #@__archives_type WHERE reid='{$topid}' And ispart<2 And ishidden<>'1'   ORDER BY   sortrank asc limit $getstart,1");
