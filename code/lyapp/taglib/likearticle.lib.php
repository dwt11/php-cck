 ...
 * 自动关连文档标签
 *
 * @version        $Id: likearticle.lib.php 1 9:29 6日
...
<demo>
{dwt:likearticle row='' col='' titlelen='' infolen=''}
...
function lib_likearticle(&$ctag,&$refObj)
{
    ...
    if( !empty($typeid) && !preg_match('#,#', $typeid) ) {
        $typeid = GetArchiveSonIds($typeid);
    }
    ...
    for($i=0; $i < $line; $i++)
    {
        ...
        for($j=0; $j < $col; $j++)
        {
            ...
            if($row = $dsql->GetArray("al"))
            {
                ...
                $row['typeurl'] = GetArchiveTypeUrl($row['typeid']);
