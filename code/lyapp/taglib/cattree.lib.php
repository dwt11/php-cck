 ...
 * 
 *
 * @version        $Id: cattree.lib.php 1 9:29 6日
...
<demo>
{dwt:cattree typeid='' catid='' showall=''/}
</demo>
<attributes>
...
function lib_cattree(&$ctag, &$refObj)
{
    ...
    {
        $row = $dsql->GetOne("SELECT reid,topid,channeltype,ispart FROM `#@__archives_type` WHERE id='$typeid' ");
        if(!is_array($row))
        {
            $typeid = $reid = $topid = $channeltype = $ispart = 0;
        } else {
    ...
    if( !empty($catid) )
    {
        $topQuery = "SELECT id,typename,typedir,isdefault,ispart,defaultname,namerule2,moresite,siteurl,sitepath FROM `#@__archives_type` WHERE reid='$catid' And ishidden<>1 ";
    }
    else
    {
        if($showall == "yes" )
        {
            $topQuery = "SELECT id,typename,typedir,isdefault,ispart,defaultname,namerule2,moresite,siteurl,sitepath FROM `#@__archives_type` WHERE reid='$topid' ";
        }
        else
        {
           if($showall=='')
           ...
           $topQuery = "SELECT id,typename,typedir,isdefault,ispart,defaultname,namerule2,moresite,siteurl,sitepath FROM `#@__archives_type` WHERE reid='{$topid}' And channeltype='{$showall}' And ispart<2 And ishidden<>1 ";
        }
    }
...
{
    global $dsql;
    $query = "SELECT id,typename,typedir,isdefault,ispart,defaultname,namerule2,moresite,siteurl,sitepath FROM `#@__archives_type` WHERE reid='{$id}' And ishidden<>1 ";
