 ...
 * Dwt织梦模板类
 *
 * @version        $Id: dwttag.class.php 1 10:33 6日
 * @package
 * @copyright
 * @license
 * @link
...
/**
 * class DwtTag 标记的数据结构描述
 * function c____DwtTag();
 *
 * @package          DwtTag
 * @subpackage
 * @link
...
class DwtTag
{
    ...
    var $EndPos = 0;        //标记结束位置
    var $CAttribute = "";   //标记属性描述,即是class DwtAttribute
    var $TagValue = "";     //标记的值
    var $TagID = 0;
    ...
    function GetName()
    {
            return $this->TagName;//原为转换为小写输出 170110改为不转换
    }
...
/**
 * DwtTagParse Dwt织梦模板类
 * function c____DwtTagParse();
 *
 * @package          DwtTagParse
 * @subpackage
 * @link
...
class DwtTagParse
{
    var $NameSpace = 'dwt';   //标记的名字空间
    var $TagStartWord = '{';   //标记起始
    var $TagEndWord = '}';     //标记结束
    var $TagMaxLen = 64;       //标记名称的最大值
    var $CharToLow = FALSE;     // TRUE表示对属性和标记名称不区分大小写
    ...
    function CheckDisabledFunctions($str, &$errmsg = '')
    {
        ...
        //clientQrCodeWeixin.htm文件使用file_put_contents获取 微信二维码 移除些函数的禁用170604
        $cfg_disable_funs = isset($cfg_disable_funs) ? $cfg_disable_funs : 'phpinfo,eval,exec,passthru,shell_exec,system,proc_open,popen,curl_exec,curl_multi_exec,parse_ini_file,show_source,fsockopen,fopen,fwrite';
    ...
    function LoadCache($filename)
    {
        ...
        $cachedir = DWTPATH . $cfg_tplcache_dir;
        ...
        if (isset($z) && is_array($z)) {
            foreach ($z as $k => $v) {
                $this->Count++;
                $ctag = new DwtTAg();
                $ctag->CAttribute = new DwtAttribute();
...
//dump("------".$filename);
            ...
            /*//引入静态文件app 160512添加     *  引入文件 app使用 160512添加 但无法使用 随后要测试
            else if ($CTag->TagName == 'includeApp') {
                $filename = ($CTag->GetAtt('file') == '' ? $CTag->GetAtt('filename') : $CTag->GetAtt('file'));
                $str = $this->IncludeFileApp($filename, $CTag->GetAtt('ismake'));
                $this->CTags[$i]->IsReplace = TRUE;
                $this->CTags[$i]->TagValue = $str;
//dump("------".$filename);
            } *///循环一个普通数组
    ...
    function RunPHP(&$refObj, $i)
    {
        ...
        if ($refObj->GetAtt('source') == 'value') {
            ...
            $DwtMeValue = $this->CTags[$i]->TagValue;
            $phpcode = $refObj->GetInnerText();
        }
        $phpcode = preg_replace("/'@me'|\"@me\"|@me/i", '$DwtMeValue', $phpcode);
        @eval($phpcode); //or die("<xmp>$phpcode</xmp>");

        $this->CTags[$i]->TagValue = $DwtMeValue;
        $this->CTags[$i]->IsReplace = TRUE;
    }
    ...
    {
        $fp = @fopen($filename, "w") or die("DwtTag 创建文件失败:  " . $filename);
            ...
            */
            ...
            if ($tTagName != '') {
                ...
                if ($e3 == -1) {
                    ...
                    //if '/}' more near '{dwt:'、'{/dwt:' , end tag is '/}', else is '{/dwt:'
                    if ($e1 < $e2 && $e1 < $e3) {
                        $endPos = $e1;
                        $elen = $endPos + strlen($eTagEndWord);
                    } else {
    ...
    {
        $DwtFieldValue = $fieldvalue;
        $functionname = str_replace("{\"", "[\"", $functionname);
        $functionname = str_replace("\"}", "\"]", $functionname);
        $functionname = preg_replace("/'@me'|\"@me\"|@me/i", '$DwtFieldValue', $functionname);
        $functionname = "\$DwtFieldValue = " . $functionname;
        @eval($functionname . ";"); //or die("<xmp>$functionname</xmp>");
        if (empty($DwtFieldValue)) {
            return '';
        } else {
            return $DwtFieldValue;
        }
    }
    ...
    function IncludeFile($filename, $ismake = 'no')
    {
        ...
        //dump($cfg_basedir);
        if (file_exists($cfg_basedir . "/web/templets/" . $filename)) {
            $okfile = $cfg_basedir . "/web/templets/" . $filename;
        } else {
            return "无法WEB/templets/找到： $filename 模板";
        }
        //编译
        if ($ismake != "no") {
    ...
    /**
     *  引入文件 app使用 160512添加 但无法使用 随后要测试
     *
     * @access    public
     *
     * @param     string $filename 文件名
    ...
    function IncludeFileApp($filename, $ismake = 'no')
    {
        global $cfg_basedir;
        $restr = '';
        if ($filename == '') {
        ...
        //dump($cfg_basedir);
        if (file_exists($cfg_basedir . "/app/templets/" . $filename)) {
            $okfile = $cfg_basedir . "/app/templets/" . $filename;
        } else {
            return "无法app/templets/找到： $filename 模板";
        }
        //编译
        if ($ismake != "no") {
            // require_once(DWTINC."/channelunit.func.php");
...
/**********************************************
 * //class DwtAttribute Dwt模板标记属性集合
 * function c____DwtAttribute();
 **********************************************/
//属性的数据描述
class DwtAttribute
{
 ...
 * function c____DwtAttributeParse();
 ********************************/
class DwtAttributeParse
{
    var $sourceString = "";
    ...
    {
        $this->cAttributes = new DwtAttribute();
        $strLen = 0;
