...
//        }
    ...
    function PreLoad()
    {
        ...
        if ($this->totalResult == 0) {
          /*   $countQuery = $this->sourceSql;
            ...
            $group_index=stripos($role_sql,"group ");//查找从左边开始第一次出现位置 如果包含group by
            $union_index=stripos($role_sql,"union");//查找从左边开始第一次出现位置 如果包含union
            if($group_index>0||$union_index>0){
                //如果包含group by则用外加COUNT
                $countQuery ="SELECT COUNT(*) AS total_count ,'1' as noroleordernumb FROM ({$role_sql}) AS SQLBYROLE";//这个外加的COUNT速度慢
            }else {
                //如果不含GROUP by 则用内加
                //171102SQL语句替换
                $strlen = strlen($role_sql);
                $from_index = stripos($role_sql, "from ");//查找从左边开始第一次出现位置   from必有空格 避免出现字段名称是from的
                //dump($from_index);
                //dump($strlen);
                $countQuery = " SELECT COUNT(*) AS total_count ,'1' as noroleordernumb  " . substr($role_sql, $from_index, $strlen);//将select 和from中的替换
                //dump($countQuery);
    ...
    function GetPageList($atts, $refObj = '', $fields = array())
    {
        ...
        if ($totalpage <= 1 && $this->totalResult > 0) {
            ...
            return "<div class=\"fixed-table-pagination\">\n<div class=\"pull-left pagination-detail\"><span class=\"pagination-info\">{$lang_total}" . $this->totalResult . $lang_record_number . "</span></div></div>";
        }
        if ($this->totalResult == 0) {
        ...
        $infos = "<div class=\"pull-left pagination-detail\"><span class=\"pagination-info\">当前第" . $this->pageNO . "页 {$lang_total}{$totalpage}{$lang_page}/{$this->totalResult}{$lang_record_number}</span> <span class=\"page-list\">每页" . $this->pageSize . "条  </span>";
        if (preg_match("#form#i", $atts['listitem'])) {
            ...
            {
                $infos .= "&nbsp;&nbsp;跳转到第<input type='text' name='pageno' style='padding:0px;width:30px;height:18px;font-size:11px' value='" . $this->pageNO . "' />页\r\n";
                $infos .= "<button  class=\"btn btn-primary btn-xs\"  type=\"submit\" > Go</button>\r\n";
            }
        }
        ...
        if (count($this->getValues) > 0) {
            foreach ($this->getValues as $key => $value) {
                //$value = urlencode($value);//180106不转码了,转码后,如果直接输入页数会识别不了汉字的搜索
                $geturl .= "$key=$value" . "&";
        ...
        //获得上一页和下一页的链接
        if ($this->pageNO != 1) {
            $prepage .= "<li class=\"page-pre\"><a class='prePage' href='" . $purl . "pageno=$prepagenum'>&lt;</a></li> \n";
            $indexpage = "<li class=\"page-first\"><a  href='" . $purl . "pageno=1'>&lt;&lt;</a></li> \n";
        } else {
            $prepage .= "<li class=\"page-pre  disabled\"><a class='prePage' href='" . $purl . "pageno=$prepagenum'>&lt;</a></li> \n";
            $indexpage = "<li class=\"page-first  disabled\"><a  href='" . $purl . "pageno=1'>&lt;&lt;</a></li> \n";
        }
        if ($this->pageNO != $totalpage && $totalpage > 1) {
            $nextpage .= "<li class=\"page-next\"> <a href='" . $purl . "pageno=$nextpagenum'>&gt;</a></li> \n";
            $endpage = "<li class=\"page-last\"><a  href='" . $purl . "pageno=$totalpage'>&gt;&gt;</a></li> \n";
        } else {
            $nextpage .= "<li class=\"page-next disabled\"> <a href='" . $purl . "pageno=$nextpagenum'>&gt;</a></li> \n";
            $endpage = "<li class=\"page-last disabled\"><a  href='" . $purl . "pageno=$totalpage'>&gt;&gt;</a></li> \n";
        }
        ...
        if ($this->pageNO >= $total_list) {
            ...
            if ($j == 0) $j = 1;//160930消除分页错误，原在第5页，或整除页时会出现 0页现象
        ...
        for ($j; $j <= $total_list; $j++) {
            $listdd .= $j == $this->pageNO ? "<li class=\"page-number active\"  > <a href='" . $purl . "pageno=$j'>" . $j . "</a></li>\n" : "<li class=\"page-number\"> <a href='" . $purl . "pageno=$j'>" . $j . "</a></li>\n";
        }

        $plist = "<div class=\"fixed-table-pagination\">\n";
        ...
        if (preg_match("#index#i", $atts['listitem'])) {
            $plist .= "<div class=\"pull-right pagination\"><ul class=\"pagination pagination-outline\">" . $indexpage;
        }
