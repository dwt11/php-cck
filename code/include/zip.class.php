 ...
 * Zip压缩类
 *
 * @version        $Id: zip.class.php 1 15:21 5日
 * @package
 * @copyright
 * @license
 * @link
...
class zip
{
    ...
    function ExtractAll ( $zn, $to)
    {
        ...
        if(is_array($files))
        {
            for($i=0;$i<$cn;$i++)
            {
                if($files[$i]['folder']==1)
                {
                    @mkdir($to.$files[$i]['filename'],$GLOBALS['cfg_dir_web_role']);
                    @chmod($to.$files[$i]['filename'],$GLOBALS['cfg_dir_web_role']);
                }
            }
        }
        $this->Extract ($zn,$to);
    ...
    function ExtractFile($header,$to,$zip)
    {
        ...
        if(!@is_dir($to))
        {
            @mkdir($to,$GLOBALS['cfg_dir_web_role']);
        }
