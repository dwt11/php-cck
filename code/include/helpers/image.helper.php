 ...
 * 图像处理相关函数
 *
 * @version        $Id: image.func.php 1 15:59 5日
...
if (!function_exists('WaterImg')) {
    function WaterImg($srcFile, $fromGo = 'up')
    {
        ...
        require_once(DWTINC . '/image.class.php');
 ...
 * @return    string
 */
if (!function_exists('ReplaceImgSrcToData_original')) {
    function ReplaceImgSrcToData_original(&$body)
    {


        preg_match_all("/<img(.*)(src=\"[^\"]+\")[^>]+>/isU", $body, $arr);
        for ($i = 0, $j = count($arr[0]); $i < $j; $i++) {
            $src_str=$arr[2][$i];
            $src_str=$src_str." ".str_replace("src="," data-original=",$arr[2][$i]);

            $body = str_replace($arr[0][$i], "<img " . $src_str . " />", $body);
        }
        return $body;
    }
}