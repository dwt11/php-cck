 ...
 * 验证小助手
 *
 * @version        $Id: validate.helper.php 2 13:56 5日 tianya $
    ...
    function Dump($var, $label=null, $strict=true, $echo=true)
    {
        ...
        if(!$strict) {
            if (ini_get('html_errors')) {
                ...
                $output = "<pre>".$label.dwt_htmlspecialchars($output,ENT_QUOTES)."</pre>";
            } else {
        ...
        if ($echo) {
            $output=str_replace("#@__",$GLOBALS['cfg_dbprefix'], $output);//替换掉SQL里的表名称
            echo($output);
            //echo "<div class=\"debug animated bounceInUp\" >$output</div>";
            return null;
        }else
            return $output;
    }
