 ...
 * 核心小助手
 *
 * @version        $Id: util.helper.php 4 19:20 6日
...
if (!function_exists('GetEditor')) {
    function GetEditor($fname, $fvalue = "", $maxnumb = 0)
    {
        ...
        //140202  添加百度编辑器
        //得到当前访问的功能名称
        $fun_name_array = explode('/', GetCurUrl());
        if (count($fun_name_array) > 1) $nowurl = $fun_name_array[1]; else $nowurl = "";
        $code = "
		  <script type='text/javascript'>
			  var dirname_plus='" . $nowurl . "';   //150901添加 fun_name  emp/archives从页面中传递过来的功能文件夹名称  用于区分 附件的保存地址
		  </script>
		 ";


        $code .= "<script id=\"$fname\" name=\"$fname\" type=\"text/plain\"  >$fvalue</script>";

        //如果同一个页面，则只加载一次配置文件
        $nowurl = ($_SERVER['PHP_SELF'] . $_SERVER["QUERY_STRING"]);
        if(empty($_SESSION['getediturl'.time()]))$_SESSION['getediturl'.time()]="";
        if ($nowurl != $_SESSION['getediturl'.time()]) {
            $code .= "   
        ...
        if ($maxnumb == 0) $code .= "  ue=UE.getEditor('$fname');";
        if ($maxnumb != 0) $code .= "  ue=UE.getEditor('$fname',{maximumWords:500});";
        $code .= "</script>";
        $_SESSION['getediturl'.time()] = $nowurl;
        return $code;
...
if (!function_exists('GetEditorSimple')) {
    function GetEditorSimple($fname, $fvalue, $maxnumb = 0)
    {
        ...
        $nowurl = ($_SERVER['PHP_SELF'] . $_SERVER["QUERY_STRING"]);
        if(empty($_SESSION['getediturl'.time()]))$_SESSION['getediturl'.time()]="";
        if ($nowurl != $_SESSION['getediturl'.time()]) $code .= "   <script type='text/javascript' src='/include/ueditor/ueditor.config_simple.js'></script><script type='text/javascript' src='/include/ueditor/ueditor.all.js'></script><script type='text/javascript' src='/include/ueditor/lang/zh-cn/zh-cn.js'></script>";

        $code .= "<script type='text/javascript'>";
        if ($maxnumb == 0) $code .= "  UE.getEditor('$fname');";
        if ($maxnumb != 0) $code .= "  UE.getEditor('$fname',{maximumWords:500});";
        $code .= "</script>";
        $_SESSION['getediturl'.time()] = $nowurl;
        return $code;
    }
    ...
    function dd2char($ddnum)
    {
        ...
        for ($i = 0; $i < $slen; $i++) {
            if (isset($ddnum[$i + 1])) {
                ...
                if (($n > 96 && $n < 123) || ($n > 64 && $n < 91)) //if( ($n>96 && $n<123) )//110717只生成小写的随机字母,未修改用LINUX主机时再考虑
                {
                    $okdd .= chr($n);
                    $i++;
                } else {
...
function IsCommend($iscommend)
{
    global $dsql;

    $arcatts = array();
    $dsql->Execute('n', 'SELECT * FROM `#@__att` ');
    while ($arr = $dsql->GetArray('n')) {
        $arcatts[$arr['att']] = $arr['attname'];
    }
    $sn = '';
    foreach ($arcatts as $k => $v) {
        //$v = cn_substr_utf8($v,1);//只要第一个字  这里取不到第一个字   注释 了  160602
        //dump($v);
