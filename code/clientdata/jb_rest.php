...
//读取归档信息
$arcQuery = "SELECT clientid,jbnum,id,createtime  FROM #@__clientdata_jblog  WHERE id='$id' ";
...
$arcQuery2 = "SELECT jbnum FROM #@__client_addon  WHERE clientid='$clientid' ";
$arcRow2 = $dsql->GetOne($arcQuery2);
$jb_ye = $arcRow2['jbnum']/100;
...
if ($dopost == 'save') {
    ...
    if ($clientid == '') {
        ...
        $jbnum100 = -$jbnum100;
        $istrue = Update_jb($clientid, "$jbnum100", "操作错误金币撤消", 0, $CUSERLOGIN->userID, "$info");
        if($istrue){echo "操作成功";}else{echo "操作失败，请检查会员的金币数量";}
        exit;
    }
}
...
<div class="wrapper wrapper-content animated fadeInRight" style="background-color: #ffffff">
    ...
    <form name="form1" id="form1" action="" method="post" class="form-horizontal">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-2">
                <span class="jbnum"><?php echo ($jb_ye) ?></span>
            </div>
        </div>
        <div class="form-group">
            <label for="" class="col-sm-2 control-label">操作金币数量</label>

            <div class="col-sm-2">
                <span class="jbnum"><?php echo -($jbnum_str) ?></span>
            </div>
        </div>
        <div class="form-group">
