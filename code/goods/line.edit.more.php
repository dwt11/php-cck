...
if ($dopost == 'save') {
    ...
    if ($goodsid == 0 || $backtime == "") {
        ShowMsg("线路名称   返回时间 不能为空 请检查！", "-1");
        exit;
    }
...
if ($dopost == '') {
    ...
    //读取归档信息
    $arcQuery = "SELECT *  FROM #@__line  WHERE id='$lineid' ";
    ...
    $query = "SELECT  
     ...
     FROM x_line  where id in ($lineids)  ORDER BY  gotime ASC
   ";

    $arcRow_11 = $dsql->GetOne($query);
    if (!is_array($arcRow_11)) {
        ShowMsg("读取信息出错!", "-1");
        exit();
    }

    $gotime_str = $arcRow_11["gotime_str"];
//dump($gotime_str);
...
-->
                ...
                $gotime_str_array = explode(",", $gotime_str);
                foreach ($gotime_str_array as $key => $gotime_str) {
                    echo "
                                            <span class=\"badge badge-success\">$gotime_str</span>
                                                 ";
