...
//默认的搜索条件
$whereSql = " WHERE ($maintable.`status`='0' OR $maintable.`status`='1')";
$searchInputFieldName_array = array("goodsname", "goodscode");//主表中 INPUT搜索框的默认字段
$searchSelectFieldName_array = array("typeid");//主表中 select搜索框的默认字段


//排序条件
$orderby = empty($orderby) ? 'weight' : $orderby;
$orderbyField = empty($orderbyField) ? '' : $orderby;
if($orderby=="weight")$orderbyField = ' ORDER BY   weight ASC';
if($orderby=="price")$orderbyField = ' ORDER BY   price DESC';
...
foreach ($searchSelectFieldName_array as $filedname) {
    ...
    if ($$filedname != "") {
        if ($filedname == "typeid") {
            ...
            $whereSql .= " AND $maintable.`typeid` IN (" . $tl->GetGoodsSonIds() . ")";    //搜索用的
        } else {
...
$leftjoinSql = "";
if ($typeid > 0) $leftjoinSql = " LEFT JOIN $addtable  on $addtable.goodsid=$maintable.id ";
...
if ($orderby == "weight") $orderbySelected = "selected";

//$searchFrom .= $af->GetSearchFrom("编号/商品名称", $searchValue_array);


//模板
if (empty($s_tmplets)) $s_tmplets = 'goods.htm';
//$s_tmplets = 'goods.htm';
