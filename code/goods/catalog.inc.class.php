 ...
 * 外部调用分类类
 *
 * @package          GoodsCatalogInc
 * @subpackage       DedeCMS.Libraries
 * @link             
 */
class goodsCatalogInc
{
    var $dsql;
    ...
    */
    ...
    {
        global $reidArray;
        $this->dsql->SetQuery("SELECT id,reid FROM #@__goods_type WHERE id='" . $id . "'    ORDER BY   sortrank ASC");
    ...
    {
        $GoodsCataloglistToMenu = array();//160606修改，输出X系统的菜单格式
        if (!$this->dsql) $this->dsql = $GLOBALS['dsql'];

        $sql = "SELECT id,typename FROM #@__goods_type WHERE reid='$typeid'";
        ...
        while ($row = $this->dsql->GetObject("goods" . $typeid)) {
            ...
            $GoodsCataloglistToMenu[$id]["title"] = $typeName;
            $GoodsCataloglistToMenu[$id]["urladd"] = "goods/goods.php?typeid=$id";

        }
        return $GoodsCataloglistToMenu;
    }
    ...
    function GetOptionListToSysFunAdd()
    {
        ...
        $query = "SELECT id,typename FROM `#@__goods_type` WHERE  reid=0   ORDER BY   sortrank ASC";
        ...
        while ($row = $this->dsql->GetObject()) {
            ...
            $funUrladd = "goods/goods.php?typeid=" . $row->id;
    ...
    function LogicGetOptionListToSysFunAdd($id, $step)
    {
        ...
        $this->dsql->SetQuery("SELECT id,typename FROM #@__goods_type WHERE reid='" . $id . "'    ORDER BY   sortrank ASC");
        $this->dsql->Execute($id);
        while ($row = $this->dsql->GetObject($id)) {
